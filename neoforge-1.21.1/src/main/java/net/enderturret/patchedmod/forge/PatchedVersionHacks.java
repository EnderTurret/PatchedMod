...
public final class PatchedVersionHacks {
	...
	static {
		...
		if (is216) {
			try {
				final Class<?> suggestCommand = Class.forName("net.minecraft.network.chat.ClickEvent$SuggestCommand",
		...
		if (is216) {
			try {
				final Class<?> showText = Class.forName("net.minecraft.network.chat.HoverEvent$ShowText",
	...
	private static void findBackgroundExecutor() {
		...
		try {
			field = Util.class.getDeclaredField("BACKGROUND_EXECUTOR");
