...
final class QuiltPlatform implements IPlatform {
	...
	public String getName(PackResources pack) {
		return pack instanceof ModNioResourcePackAccess mod ? "mod/" + mod.getModInfo().name() : pack.getName();
	}
	...
	public Function<ResourceLocation, ResourceLocation> getRenamer(PackResources pack, String namespace) {
		// GroupResourcePack, ModNioResourcePack, and VanillaPackResources
		// (Apparently Quilt does something that fixes VPR?)
		if (isGroup(pack) || pack instanceof ModNioResourcePack || pack instanceof VanillaPackResources)
			return rl -> rl;
